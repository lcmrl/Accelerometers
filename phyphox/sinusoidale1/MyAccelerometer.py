...
### INPUT PARAMETERS ###
acc_data_csv = r"./phyphox/sinusoidale2.csv"
separator = ";"
NUMBER_STATIC_EPOCHS = int(14.7/0.01)

# Butterworth low pass filter
sample_rate = 100 # Hz
cutoff = 1 # desired cutoff frequency of the filter in Hz
nyq = 1.0 * sample_rate  # nyquist frequency
order = 2
...
def NumIntegration(input_array, time_array):
    ...
    print(input_array)
    print(time_array)
    print(len(input_array), len(time_array))
...
plt.plot(TIME[:], AX[:], 'b-', TIME[:], VX[:], 'r-')#, TIME, SX, 'g-') #plt.scatter(TIME, AX, color='r',marker=".", s=1)
plt.show()
